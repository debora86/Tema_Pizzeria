...
Plugin URI:
Description: Añade Postype al sitio web
Version:1.0.0
Author: Debora
Author URI:
...
// // agregando campos a la repuesta de la api
function lapizzeria_agregar_campos_rest_api(){
	register_rest_field( 
		'especialidades',
		'precio',

		 array( 'get_callback' => 'la_pizzaria_precio',
	...
	register_rest_field( 
		...
		array( 
			'get_callback' => 'lapizzaria_imagen_destacada',
		 	'update_callback' => null,
		 	'schema' => null
		) 
	);
...
// // para saber si tienen habilitado el plugins avenced custom field
function la_pizzaria_precio(){
 ...
 function lapizzaria_imagen_destacada($object, $field_name, $request ) {
